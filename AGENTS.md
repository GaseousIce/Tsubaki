...
- `src/anti_phishing/` — 5 files:
  ...
  - `actions.py` — detection pipeline execution: immediate message deletion, DB logging with content and attachment metadata, user recovery DM, punishment execution (`timeout`/`kick`/`ban`/`warn`), and moderator alerts with message previews, embed summaries, and attachment metadata evidence. `PhishingAlertView` provides interactive action buttons (False Positive, Ban, Timeout, Unban).
